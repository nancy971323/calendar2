...
export default {
  ...
  setup() {
    ...
    // 計算屬性
    const _currentUser = computed(() => authStore.currentUser)
    const employees = computed(() => employeeStore.employees)
    const _securityLevels = computed(() => employeeStore.securityLevels)
    ...
    const saveNewEmployee = async () => {
      try {
        ...
        // 創建新員工
        await employeeStore.createEmployee(newEmployee.value)
    ...
    const saveEditedEmployee = async () => {
      try {
        ...
        // 更新員工信息
        await employeeStore.updateEmployeeById({
        ...
        if (editingEmployeePassword.value) {
          await employeeStore.updateEmployeePassword({
            id: editingEmployee.value.id,
    ...
    const loadSecurityLevels = async () => {
      try {
        ...
        // 修正安全等級數據轉換方式
        if (employeeStore.securityLevels) {
          if (Array.isArray(employeeStore.securityLevels)) {
            // 如果是數組格式
            securityLevelOptions.value = employeeStore.securityLevels.map(level => ({
              name: level.description,
              value: level.code
            }));
          } else {
            // 如果是對象格式 (key-value 形式)
            securityLevelOptions.value = Object.keys(employeeStore.securityLevels).map(key => ({
              name: employeeStore.securityLevels[key].description,
              value: key
            }));
          }
        }
